...
def get_question_query(qid, question, char_idx, wiki_paragraphs=False, sent_idx = None):
    ...
    if wiki_paragraphs:
        print(question['text'][:char_idx])
        print(question['annotated_paras'][:sent_idx])
    ...
    for question_idx, q in enumerate(tqdm(questions)):
        ...
        if not IS_MULTI:
            ...
            for i in range(len(length_array)):
                query = get_question_query(question_idx, q, length_array[i], sent_idx = i)
                resp = requests.post(url, json=query).json()
                query.update(resp)
                answers[-1].append(query)
    return answers
            ...
            for idx in range(max_sentences):
                ...
                for i, q in enumerate(qs):
                    if idx < len(length_array[i]):
                        query['questions'].append(
                            get_question_query(qids[i], q, length_array[i][idx], sent_id = idx))
                resp = requests.post(url, json=query).json()
                for i, r in enumerate(resp):
                    q = query['questions'][i]
                    q.update(r)
